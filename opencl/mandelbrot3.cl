...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/11 17:24:39 by viccarau          #+#    #+#             */
/*   Updated: 2019/07/11 18:08:57 by viccarau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
__kernel void	render(__global int *out, t_krn k, int mx, int my)
{
	...
	i = 0;
	xtemp = m.x * m.x * m.x - 3 * m.x * m.y * m.y + m.x_origin;
	while (m.x * m.x + m.y * m.y <= 4 && i < k.iter)
	{
		xtemp = m.x * m.x * m.x - 3 * m.x * m.y * m.y + m.x_origin;
		m.y = 3 * m.x * m.x * m.y - m.y * m.y * m.y + m.y_origin;
		m.x = xtemp;
		i++;
	}
	if (i == k.iter)
