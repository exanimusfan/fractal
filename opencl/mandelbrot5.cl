...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/11 18:08:34 by viccarau          #+#    #+#             */
/*   Updated: 2019/07/11 18:17:15 by viccarau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
__kernel void	render(__global int *out, t_krn k, int mx, int my)
{
	...
	i = 0;
	xtemp = m.x * m.x * m.x * m.x * m.x - 10 *
		m.x * m.x * m.x * m.y * m.y + 5 * m.x * m.y * m.y * m.y* m.y + m.x_origin;
	while (m.x * m.x + m.y * m.y <= 4 && i < k.iter)
	{
		xtemp = m.x * m.x * m.x * m.x * m.x - 10 *
			m.x * m.x * m.x * m.y * m.y + 5 * m.x * m.y * m.y * m.y* m.y + m.x_origin;
		m.y = 5 * m.x * m.x * m.x * m.x * m.y - 10 *
			m.x * m.x * m.y * m.y * m.y + m.y * m.y * m.y * m.y * m.y + m.y_origin;
		m.x = xtemp;
		i++;
	}
	if (i == k.iter)
